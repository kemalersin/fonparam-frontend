...
export default function Funds() {
    ...
    const [comparisonStates, setComparisonStates] = useState<Record<string, boolean>>({});
    const [checkingFavorites, setCheckingFavorites] = useState<Record<string, boolean>>({});
    const [checkingComparison, setCheckingComparison] = useState<Record<string, boolean>>({});
    ...
    useEffect(() => {
        if (!isInitialized) {
            ...
            if (companyFromUrl) setCompanyFilter(companyFromUrl);
    ...
    // URL parametrelerini sessizce güncelle
    useEffect(() => {
        if (isInitialized) {
    ...
    }, [search, page, sort, order, companyFilter, setSearchParams, isInitialized]);
    ...
    const { data, isLoading } = useFunds({
        ...
        sort,
        order,
        management_company: companyFilter || undefined,
    });
    ...
    const handleComparisonClick = async (event: React.MouseEvent, fund: any) => {
        ...
        const fundCode = fund.code;
        setCheckingComparison(prev => ({ ...prev, [fundCode]: true }));

        try {
            if (comparisonStates[fundCode]) {
                await removeFromComparison(fundCode);
                setComparisonStates(prev => ({ ...prev, [fundCode]: false }));
                setToast({
                    show: true,
                    type: 'info',
                ...
                setComparisonStates(prev => ({ ...prev, [fundCode]: true }));
                setToast({
                    show: true,
                    type: 'success',
            ...
            setCheckingComparison(prev => ({ ...prev, [fundCode]: false }));
        }
    };
        ...
        if (data?.data) {
            const checkStates = async () => {
                ...
                for (const fund of data.data) {
                    ...
                    setCheckingComparison(prev => ({ ...prev, [fund.code]: true }));
                    ...
                    setCheckingComparison(prev => ({ ...prev, [fund.code]: false }));
                }

                setFavoriteStates(favoriteResults);
                setComparisonStates(comparisonResults);
            };
            checkStates();
        }
    }, [data?.data]);
